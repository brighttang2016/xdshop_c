...
angular.module('com.app.publish.controller')
    .controller('PublishImageModalFirstShareController', function ($uibModalInstance,item,$scope,$rootScope,PublishService,TlmsRestangular) {
    $scope.item =item;//包含：openId，publishId
    //var branchId = item.branchId;
    //var title = item.title;
    //var applyEdit = item.applyEdit;
    //
    $scope.initFirstShare = function(){
        $scope.userInfo = {};
        // $scope.showUserInfo();
        $scope.loading = PublishService.generalFirstSharePic(item.id).then(function(response){
            console.log(response);
            if(response.successResponse == true){
                $scope.sharePicUrl = response.data.pushPosterUrl;
            }else{
